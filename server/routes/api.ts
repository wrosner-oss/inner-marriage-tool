...
api.get('/library', wrap(async (_req: any, res: any) => {
  ...
  res.json({
    signs: signs.map((s) => ({
      ...
      feminineSupport: parseJsonArray(s.feminineSupport),
...
api.put('/library/signs/:name', wrap(async (req: any, res: any) => {
  ...
  if ('feminineSupport' in b) data.feminineSupport = Array.isArray(b.feminineSupport) && b.feminineSupport.length ? JSON.stringify(b.feminineSupport) : null;
